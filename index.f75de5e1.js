...
function checkData(formData) {
    if (formData.name.length < 4) {
        pushNotification(500, 10, "Error!", `Something went wrong!<br> ` + `Name field cannot be empty! Or less then 4 letters!`, "error");
        return false;
    }
    if (!formData.position) {
        pushNotification(500, 10, "Error!", `Something went wrong!<br> ` + `Position field cannot be empty!`, "error");
        return false;
    }
    if (+formData.age < 18 || +formData.age > 90) {
        pushNotification(500, 10, "Error!", `Something went wrong!<br> ` + `Age must be more 18 & less 90`, "error");
        return false;
    }
    return true;
}
...
panel.addEventListener("submit", (e)=>{
  ...
  `;
    if (checkData(data) === true) {
        pushNotification(500, 10, "Success!", "Good!\n Employee has been added!", "success");
...
function sortList(list, index, sortMethod) {
    const result = [
        ...
        if (index === 4) {
            ...
            if (sortMethod === "down") return valueB - valueA;
            return valueA - valueB;
        }
        const value1 = a.children[index].innerText;
        const value2 = b.children[index].innerText;
        if (sortMethod === "down") return value2.localeCompare(value1);
...
addEventListener("click", (e)=>{
    ...
    const i = [
        ...tableHeaders
    ].findIndex((element)=>element.innerText === e.target.innerText && !tableFooter.contains(e.target));
    let k;
    if (i === -1) return;
    for (const element of tableHeaders)if (element.hasAttribute("data-status") && element !== tableHeaders[i]) element.removeAttribute("data-status");
...
addEventListener("click", (e)=>{
    const tableBody = document.querySelector("tbody");
    if (!tableBody.contains(e.target)) return;
    for (const element of tableBody.children)if (element.classList.contains("active")) element.classList.remove("active");
...
addEventListener("dblclick", (e)=>{
    const tableBody = document.querySelector("tbody");
    if (!tableBody.contains(e.target)) return;
    if (document.querySelector(".cell-input")) document.querySelector(".cell-input").parentElement.innerText = memory.innerText;
    const i = [
    ...
    if (i > 2) plaseText.innerHTML = `<input
      class="cell-input" name="data" type="number" value="">`;
    const input = document.querySelector(".cell-input");
    input.addEventListener("keypress", (ev)=>{
        if (ev.key !== "Enter") return;
        if (ev.target.value.length === 0) plaseText.innerText = memory.innerText;
        if (i === 0 && ev.target.value.length < 4) {
            input.reset();
            return;
        }
        if (i === 3 && (Number(ev.target.value) < 18 || Number(ev.target.value) > 90)) {
            input.reset();
            return;
        }
